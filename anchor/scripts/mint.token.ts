...
import { getMint, getOrCreateAssociatedTokenAccount, mintTo, TOKEN_2022_PROGRAM_ID } from '@solana/spl-token'
...
program
  ...
  .requiredOption('-m, --mint-address <string>', 'Address of token mint account')
  ...
  .action(async (options: MintTokenOptions) => {
    try {
      ...
      const receiverBalance = await connection.getTokenAccountBalance(receiverATA.address)
      console.table({
